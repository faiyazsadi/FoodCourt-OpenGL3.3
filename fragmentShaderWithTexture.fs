...
vec3 CalcDirLight(Material material, DirectionLight light, vec3 N, vec3 fragPos, vec3 V);
...
vec3 CalcSpotLight(Material material, SpotLight light, vec3 N, vec3 fragPos, vec3 V)
{
    ...
    vec3 V_l = -normalize(light.position-vec3(-10,-1.0,-2.0)); // incident light position
